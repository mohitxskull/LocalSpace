 ...
 *
 * @example
 * // Create a root cache for a user
 * const userCache = ScopedCache.create(cache.driver(), {
 * key: `user:${userId}`,
 * ttl: '1h',
 * factory: () => fetchUserFromDb(userId),
 * });
 *
 * // Get the user, fetching from DB and caching if not present
 * const user = await userCache.get();
 *
 * // Extend to create a child cache for the user's posts
 * const postsCache = userCache.extend('posts', (user) => {
 * return fetchPostsForUser(user.id);
 * });
 *
 * // Get the posts, which first ensures the user is cached, then fetches posts
 * const posts = await postsCache.get();
 */
export class ScopedCache<T> {
  ...
  /**
   * Creates a new child ScopedCache in a nested namespace.
   * This is powerful for creating hierarchies of related cached data.
   *
  ...
  /**
   * Creates a new child ScopedCache in a nested namespace WITHOUT a data dependency.
   * The child's factory is independent and does not receive the parent's value.
   *
   * Use this for logically grouping related but independent caches (e.g., settings -> ui-settings).
   *
   * @template U The data type of the new child cache.
